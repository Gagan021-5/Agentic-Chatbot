    ...
    <div className={`card-hover relative flex h-full flex-col rounded-2xl border p-4 shadow-soft sm:rounded-[22px] sm:p-5 ${
      ...
      <div className="mt-1 flex items-start justify-between gap-3 sm:mt-2">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-sm leading-7 text-white/50 sm:mt-3 sm:text-base sm:leading-8">{plan.desc}</p>
        </div>
        <div className="shrink-0 rounded-2xl border border-white/8 bg-black/20 px-3 py-2 text-right sm:px-4 sm:py-3">
          <div className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-full bg-yellow-500" />
            <span className="text-2xl font-extrabold text-white sm:text-3xl">{plan.joules.toLocaleString()}</span>
          </div>
          <div className="mt-0.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-white/40 sm:text-xs">joules</div>
          <div className="mt-1 text-xs text-white/45 sm:text-sm">${plan.usd}</div>
        </div>
      </div>

      <div className="mt-4 flex flex-wrap gap-2 sm:mt-5">
        <span className="rounded-full border border-white/8 bg-white/4 px-3 py-1 text-xs text-white/50">⊙ 7 scope items</span>
        <span className="rounded-full border border-white/8 bg-white/4 px-3 py-1 text-xs text-white/50">⊙ 1 week</span>
      </div>
      <div className="mt-2">
        <span className="rounded-full border border-white/8 bg-white/4 px-3 py-1 text-xs text-white/50">⊙ AI-assisted with human review</span>
      </div>

      <div className="mt-4 rounded-xl border border-white/6 bg-black/15 p-3 sm:mt-5 sm:rounded-2xl sm:p-4">
      ...
      <button
        ...
        className={`mt-4 h-10 w-full rounded-xl text-sm font-bold transition sm:mt-5 sm:h-12 ${
          featured
            ? "btn-cta text-white"
